    ...
    while (($module = $dir->read()) !== false) {
      if(is_dir("$src/$module")) {
        if(class_exists($module)) {
          ...
          $modules[$module]['isLydiaCore']   = in_array($rc->name, array('Lydia', 'Database', 'Request', 'ViewContainer', 'Session', 'Object'));
          $modules[$module]['isLydiaCMF']    = in_array($rc->name, array('Form', 'Page', 'Blog', 'MUser', 'User', 'MContent', 'Content', 'FormUserLogin', 'FormUserProfile', 'FormUserCreate', 'FormContent', 'CHTMLPurifier'));
        }
      }
    }
    $dir->close();
